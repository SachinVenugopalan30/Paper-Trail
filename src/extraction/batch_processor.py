...
class BatchProcessor:
    ...
    def _process_single_pdf(self, pdf_path: str, limit_pages: Optional[int] = None) -> Dict:
        """
        ...
        try:
            ...
            try:
                ...
                self.checkpoint.mark_file_failed(pdf_path, f"Could not get page count: {e}", "page_count")
            ...
            pdf_images_dir = self.images_dir / pdf_name if self.save_images else None
            ...
            try:
                ...
                self.checkpoint.mark_file_failed(pdf_path, f"PDF conversion failed: {e}", "conversion")
            ...
            try:
                ...
                self.checkpoint.mark_file_failed(pdf_path, f"Native extraction failed: {e}", "native")
            ...
            pages_to_process = min(len(image_paths), limit_pages) if limit_pages else len(image_paths)
            
            for page_idx, image_path in enumerate(image_paths[:pages_to_process], start=1):
                ...
                try:
                    if page_idx <= len(native_pages):
                        ...
                        page_result["native"]["processing_time_ms"] = int(native_time_total / len(native_pages)) if native_pages else 0
                        
                        native_success += 1
                        total_coverage += page_data.get("coverage", 0.0)
                    else:
                        page_result["native"]["error"] = f"Page {page_idx} not found in native extraction"
    ...
    def process_batch(
        ...
        print(f"Processing {len(files_to_process)} PDFs with {self.parallel_workers} workers...")
    ...
    def get_failed_files(self) -> Dict[str, str]:
        ...
        return {
            k: v["error"] 
            for k, v in self.checkpoint.get_failed_files().items()
        }
    ...
    def get_skipped_files(self) -> Dict[str, str]:
        ...
        return {
            k: v["reason"] 
            for k, v in self.checkpoint.get_skipped_files().items()
        }
