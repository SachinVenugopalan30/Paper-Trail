...
echo $WORKDIR
# Wrap the entire expansion in quotes to keep the space "glued" together
...
NEO4J_SIF="${NEO4J_SIF:-/scratch/$USER/containers/neo4j.sif}"  # apptainer pull neo4j.sif docker://neo4j:5-community
OCR_HOST="${OCR_HOST:-localhost}"
OCR_PORT="${OCR_PORT:-8080}"

# GLM-OCR via vLLM on NVIDIA GPU (Sol sgh nodes)
...
GLM_OCR_GPU="${GLM_OCR_GPU:-0}"   # CUDA device index to use for GLM-OCR
...
# ============================
# Start GLM-OCR server via vLLM on Gaudi (only needed for ocr/hybrid)
# ============================
