...
# vLLM Gaudi container
VLLM_SIF="${VLLM_SIF:-/data/sse/gaudi/containers/vllm-gaudi.sif}"
...
TB_SCRATCH="$SCRATCH_BASE/paper_trail/$JOB_ID"
mkdir -p "$TB_SCRATCH"/{hf,vllm,torch,xdg,tmp,habana_logs}
...
# ============================
SCRATCH_BASE_NEO4J="${SCRATCH:-/scratch/$USER}"
NEO4J_SCRATCH="$SCRATCH_BASE_NEO4J/neo4j/persistent"
...
# ============================
# Start GLM-OCR server via vLLM on Gaudi HPU (only needed for ocr/hybrid)
# ============================
if [[ "$EXTRACT_METHOD" != "native" ]] && [[ "$RUN_EXTRACT" == "1" ]]; then
    if ! curl -fsS "http://${OCR_HOST}:${OCR_PORT}/v1/models" >/dev/null 2>&1; then
        echo "Starting GLM-OCR server ($GLM_OCR_MODEL) via vLLM on Gaudi HPU..."
        ...
        wait_for_http "http://${OCR_HOST}:${OCR_PORT}/v1/models" "GLM-OCR vLLM (Gaudi)" 1800
        echo "GLM-OCR ready at http://${OCR_HOST}:${OCR_PORT}"

        # Smoke test
        echo "Smoke testing GLM-OCR..."
        set +e
        curl -fsS "http://${OCR_HOST}:${OCR_PORT}/v1/models" >/dev/null
        rc=$?
        set -e
        if [[ $rc -ne 0 ]]; then
            echo "WARNING: GLM-OCR smoke test failed. Check $LOGDIR/ocr_server.log"
            tail -100 "$LOGDIR/ocr_server.log" || true
        fi
    else
...
# Copy scratch logs to persistent storage
cp -f "$TB_SCRATCH/"*.log "$LOGDIR/" 2>/dev/null || true
